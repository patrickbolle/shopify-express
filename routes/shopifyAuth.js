  ...
  return {
    ...
    auth(request, response) {
      ...
      if (!request.cookies[TOP_LEVEL_OAUTH_COOKIE_NAME]) {
        ...
        topLevelRedirect(response, `${redirectTo}?${querystring.stringify(redirectParams)}?referrer=` + referrer);
        return;
      }

      response.clearCookie(TOP_LEVEL_OAUTH_COOKIE_NAME);
      response.redirect(`${redirectTo}?${querystring.stringify(redirectParams)}?referrer=` + referrer);
    },
